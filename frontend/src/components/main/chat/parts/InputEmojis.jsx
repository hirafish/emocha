...
const InputEmojis = () => {
  ...
  const tailwindShowAttention =
    "absolute bottom-24 bg-opacity-75 bg-white w-full shadow rounded-lg";
  ...
  // 送信処理
  const sendChatData = async (postData, userId) => {
    if (userId === "") {
      alert("送信するにはログインしてください！");
      return;
    }
    try {
    ...
    <div className="flex-2 pt-4 pb-6 relative">
      <div id="picker" className={showEmojiPicker}>
        <Picker
          ...
          locale={userSettings.language === "English" ? "en" : "ja"}
          noCountryFlags={true}
          set="twitter"
      ...
      <div className="write bg-gray-50 border border-gray-200 shadow flex rounded-lg max-h-20 dark:bg-gray-700 dark:border-gray-800">
        ...
        <div
          className="textarea flex-1 min-w-0 max-h-16 cursor-text"
          onClick={(event) => {
        ...
        <div className="flex-2 w-32 p-2 flex content-center items-center relative">
          <div className="flex-1 text-center">
            <span
              ...
              className="text-gray-400 hover:text-gray-500 cursor-pointer dark:text-gray-100 dark:hover:text-gray-300"
            >
          ...
          <div className="flex-1">
            <button
              ...
              className="bg-blue-600 hover:bg-blue-700 w-10 h-10 rounded-full inline-block"
            >
